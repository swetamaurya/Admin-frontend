...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await adminApi.getUsers(currentPage, 5, searchTerm);
      
      if (response && response.data) {
        // Map isActive to isBlocked for frontend compatibility
  ...
  const confirmToggleBlock = async () => {
    try {
      const response = await adminApi.updateUserStatus(userToBlock._id);
